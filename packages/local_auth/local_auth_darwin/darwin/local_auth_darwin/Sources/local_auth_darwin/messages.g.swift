...
struct AuthOptions: Hashable {
  ...
  static func == (lhs: AuthOptions, rhs: AuthOptions) -> Bool {
    ...
    return deepEqualsmessages(lhs.biometricOnly, rhs.biometricOnly) && deepEqualsmessages(lhs.sticky, rhs.sticky) && deepEqualsmessages(lhs.checkBiometricInvalidationForKey, rhs.checkBiometricInvalidationForKey)
  }
  ...
  func hash(into hasher: inout Hasher) {
    ...
    deepHashmessages(value: checkBiometricInvalidationForKey, hasher: &hasher)
  }
}
...
protocol LocalAuthApi {
  ...
  ///
  /// Unlike [getEnrolledBiometrics] this reflects the physical sensor, so it
  /// reports e.g. [AuthBiometric.face] on a Face ID device even before the user
  /// has set up Face ID. (Apple biometrics are always Secure Enclave-backed, so
  /// there is no weak/strong distinction to report on iOS.)
...
class LocalAuthApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: LocalAuthApi?, messageChannelSuffix: String = "") {
    ...
    /// has set up Face ID. (Apple biometrics are always Secure Enclave-backed, so
    /// there is no weak/strong distinction to report on iOS.)
    let getHardwareBiometricCapabilitiesChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.local_auth_darwin.LocalAuthApi.getHardwareBiometricCapabilities\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      getHardwareBiometricCapabilitiesChannel.setMessageHandler { _, reply in
        do {
          let result = try api.getHardwareBiometricCapabilities()
          reply(wrapResult(result))
        } catch {
          reply(wrapError(error))
        }
      ...
      getHardwareBiometricCapabilitiesChannel.setMessageHandler(nil)
    }
    /// Attempts to authenticate the user with the provided [options], and using
    /// [strings] for any UI.
